...
public:
	astar_planning(string name):Node(name){
		...
		// 地图数据发布节点
		result_data_publisher = this->create_publisher<nav_msgs::msg::Path>("path", 10);
		timer = this->create_wall_timer(std::chrono::seconds(1), std::bind(&astar_planning::timer_callback, this));
